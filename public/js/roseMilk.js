...
let cName = "", currentStream, count = 1, closeVideo, takeSnap, videoRecorder, closeVideoFromStream, toSendPhoto, toSendVideo, toMakeGif, toRunMe, toSendGif, toSendMap, tempData, tempCords, moreThanOne = false, cameraArr, cameraRotate, toRemoveCameraIcon = false, deleteTrashArr = [], deleteItemsFromList
let myRecorder = {
  objects: {
    context: null,
    stream: null,
...
function init() {
  ...
  if (count === 2 && message !== "") {
    ...
    if (message.match(pattern1)) {
      return socket.emit("chat", { message: message, handle: cName, id: idnumber() }), $('#msg').val("");
    }
    ...
    if (message.split(" ").join("") === 'shownew') return socket.emit("showNewfeature"), $('#msg').val("")
      ...
      if (validator.isURL(message)) return axios.post("https://abts-link-preview.herokuapp.com/api", { Url: message }).then(res => { socket.emit("chat", { message: "linkPreview", handle: cName, id: idnumber(), info: res.data.result }) }), $('#msg').val("")
      let clean = validator.escape(message)
      var encrypted = encrypt(clean, password);
      socket.emit("chat", { message: encrypted, handle: cName, id: idnumber() });
    }
    $('#msg').val("")
  }
}
    ...
    delivery.on('delivery.connect', function (delivery) {
      $("#sendfile").on('change', function (evt) {
        ...
        if (Number(maxSize) > file.size) {
          var extraParams = { type: file.type ? file.type : "text/plain", tag: file.type.split('/')[0], extra: file.type.split('/').pop(), handle: cName, id: idnumber() };
  ...
  if (null !== listObject && 'object' === typeof listObject && listObject.length > 0) {
    myRecorders.objects.recorder.exportWAV(function (blob) {
      ...
      if (Number(maxSize) > bs) {
        socket.emit("blob", { blob: blob, handle: cName, type: blob.type, sending: "audio", id: idnumber() })
        $('#audioTemp').empty()
...
async function toSendGiffuc() {
  ...
  await socket.emit('blob', { blob: tempData, handle: cName, type: 'image/gif', sending: "gif", id: idnumber() })
...
async function toSendPhotofuc() {
  ...
  await socket.emit('blob', { blob: tempData, handle: cName, type: 'image/jpeg', sending: "image", id: idnumber() })
...
async function toSendVideofuc() {
  ...
  await socket.emit('blob', { blob: tempData, handle: cName, type: tempData.type, sending: "video", id: idnumber() })
...
async function toSendMapfuc() {
  ...
  await socket.emit('blob', { blob: mapImage, handle: cName, type: encrypt(tempCords, password), sending: "map", extra: "image/png", id: idnumber() })
...
function toBeDeleted() {
  return deleteTrashArr.forEach(element => { socket.emit("deleteList", element) }), deleteTrashArr = [], deleteItemsFromList = undefined, deleteTrashRemove()
}
