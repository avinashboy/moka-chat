...
let cName = "", currentStream, count = 1, closeVideo, takeSnap, videoRecorder, closeVideoFromStream, toSendPhoto, toSendVideo, toMakeGif, toRunMe, toSendGif, tempData, moreThanOne = false, cameraArr, cameraRotate, toRemoveCameraIcon = false
let myRecorder = {
  objects: {
    context: null,
    stream: null,
...
function init() {
  ...
  if (count === 2 && message !== "") {
    ...
    if (message.split(" ").join("") === 'appversion') return alert("secondary", `App Version = ${appVersion}`)
...
socket.on("disconnect", () => {
  $('#send').append(`<p class="dummy text-danger">You has left chat </p>`);
})
...
function closeVideoFromStreamfuc() {
  closeVideoFromStream = toSendPhoto = toSendVideo = toSendGif, toRunMe = undefined
